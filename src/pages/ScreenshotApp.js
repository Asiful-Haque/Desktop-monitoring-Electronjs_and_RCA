...
//   // timing + segments
//   const startAtRef = useRef(null);          // current open segment start
//   const segmentsRef = useRef([]);           // collected closed segments { startAt: Date, endAt: Date }
...
//     const dt = d instanceof Date ? d : new Date(d);
//     return `${formatDateYMD(dt)} ${pad(dt.getHours())}:${pad(dt.getMinutes())}:${pad(dt.getSeconds())}`;
//   };
...
//   // token (optional) + initial tasks
//   useEffect(() => {
//     window.electronAPI
//       .getTokenCookie()
//       .then((fetchedToken) => {
//         if (fetchedToken) setToken(fetchedToken);
//         fetchData(); // with credentials: 'include', cookie will be sent automatically
//       })
//       .catch((err) => {
...
//   // SUBMIT (handles both single + multi)
//   const handleFinish = async () => {
//     // if currently running, close the segment
//     if (startAtRef.current) {
//       segmentsRef.current.push({
//         startAt: new Date(startAtRef.current),
//         endAt: new Date(),
...
//     const theTask = taskData.find((t) => String(getTaskId(t)) === String(selectedTaskId));
//     if (!theTask) {
...
//     // If exactly one segment, you can send single object; else send array
//     const bodyToSend = rows.length === 1 ? rows[0] : rows;

//     try {
//       // POST time-tracking (createOne or createMany)
//       console.log("Submitting time tracking:----------------------", bodyToSend);
//       const ttRes = await fetch(`${process.env.REACT_APP_API_BASE}/api/time-tracking`, {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         credentials: "include",
//         body: JSON.stringify(bodyToSend),
//       });
//       console.log("TimeTracking response status:", ttRes.status);
//       const ttData = await ttRes.json();
//       if (!ttRes.ok) {
//         console.error("TimeTracking insert error:", ttData);
//         showToast(ttData?.error || "Failed to submit time tracking.");
//         setShowFinishConfirm(true);
//         return;
//       }

//       // Update last_timing on the Task
...
//       if (!updateRes.ok) {
//         console.warn("Task update (last_timing) failed:", updateData);
//       }

//       console.log("✅ TimeTracking saved:", ttData);
...
//           <button id="screenshotBtn" className="button is-success" onClick={handleStart}>
//             Start
//           </button>
//         )}
//         <button id="stopBtn" className="button is-danger" onClick={isPaused ? handleResume : handlePause}>
//           {isPaused ? "Resume" : "Pause"}
//         </button>

//         <button id="finishBtn" className="button is-danger" onClick={handleFinish}>
//           Submit Task
//         </button>
//       </div>
